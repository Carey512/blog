    ...
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-3 pb-8 pt-4 sm:px-5 lg:px-6">
      <section className="grid gap-4 border-b border-border pb-4 lg:grid-cols-[minmax(0,1fr)_220px]">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-muted">{t.allArticlesIntro}</p>
        </div>
        <div className="rounded-lg border border-border bg-surface p-4 shadow-line">
          <span className="block text-3xl font-semibold text-primary">{posts.length}</span>
          <span className="mt-1 block text-sm text-muted">{t.articleCount}</span>
        </div>
      </section>
