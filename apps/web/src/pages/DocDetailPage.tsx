import { ArrowLeft, BookOpenText, CheckCircle2, Download } from 'lucide-react';
...
export function DocDetailPage() {
  ...
  const docHtmlFile = doc.htmlFile ?? '';
  const docHtmlUrl = docHtmlFile ? docsService.getDocHtmlUrl(docHtmlFile) : '';
  const docHtmlDownloadUrl = docHtmlFile ? docsService.getDocHtmlDownloadUrl(docHtmlFile) : '';

  async function handleDownloadHtml() {
    if (!docHtmlFile || !docHtmlDownloadUrl) {
      return;
    }
    ...
    try {
      const response = await fetch(docHtmlDownloadUrl);
      ...
      link.href = objectUrl;
      link.download = docHtmlFile;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(objectUrl);
    } catch {
      setDownloadError(t.docsDownloadError);
    }
  }
        ...
        <div className="p-3 sm:p-4">
          {docHtmlUrl ? (
            <section className="overflow-hidden rounded-lg border border-border bg-background shadow-line">
              ...
              {downloadError ? (
                <p className="border-b border-border bg-background px-3 py-2 text-xs font-semibold text-primary">
                  {downloadError}
                </p>
              ) : null}
              <iframe
